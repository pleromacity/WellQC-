    ...
    <main className="min-h-screen bg-wellqc-dark flex items-center justify-center p-5">
      <div className="w-full max-w-md border border-wellqc-border bg-wellqc-panel p-7 rounded-xl shadow-2xl">
        <div className="flex items-center gap-3 mb-7">
          <div className="w-10 h-10 rounded-lg bg-cyan-500/15 border border-cyan-500/30 flex items-center justify-center"><Database className="w-5 h-5 text-cyan-300" /></div>
          <div><h1 className="text-xl font-black text-white">WellQC+</h1><p className="text-xs text-wellqc-muted font-mono">Well log quality workspace</p></div>
        </div>
        <h2 className="text-lg font-bold text-white">{isRegister ? "Create your account" : "Sign in"}</h2>
        <p className="mt-1 text-xs text-wellqc-muted">{isRegister ? "Start validating and managing your well logs." : "Use your WellQC+ account to continue."}</p>
        ...
        <p className="mt-5 text-center text-xs text-wellqc-muted">{isRegister ? "Already have an account?" : "New to WellQC+?"} <Link href={isRegister ? "/login" : "/register"} className="text-cyan-300 hover:text-cyan-200 font-semibold">{isRegister ? "Sign in" : "Create an account"}</Link></p>
      </div>
    </main>
  );
}
