...
export async function POST(request: Request) {
  try {
    const { name, email, password, acceptedNda } = await request.json() as { name?: string; email?: string; password?: string; acceptedNda?: boolean };
    ...
    const user = await db.user.create({
      data: { name: safeName, email: safeEmail, passwordHash: await hashPassword(password), ndaAcceptedAt: new Date() },
    });
